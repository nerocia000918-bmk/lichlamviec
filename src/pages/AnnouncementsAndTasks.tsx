...
export default function AnnouncementsAndTasks({ user }: { user: User | null }) {
  ...
  const fetchData = async () => {
    try {
      const [annRes, taskRes, empRes] = await Promise.all([
        ...
        fetch(`/api/assigned-tasks?employee_id=${user?.id || ''}&department=${encodeURIComponent(user?.department || '')}&role=${encodeURIComponent(role)}`),
        fetch('/api/employees')
      ]);
      
      if (!annRes.ok || !taskRes.ok || !empRes.ok) throw new Error('Fetch failed');
  ...
  useEffect(() => {
    if (activeTab === 'tasks' && user && Array.isArray(tasks)) {
      tasks.forEach(task => {
        if (task && task.viewed_at === null) {
                  ...
                  <div className="flex-1 space-y-3">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-xs text-slate-400 flex items-center gap-1">
                        ...
                        {task.created_at ? format(parseISO(task.created_at), 'dd/MM/yyyy HH:mm') : 'N/A'}
                      </span>
                ...
                <div className="p-5 flex flex-col md:flex-row justify-between gap-4">
                  <div className="flex-1 space-y-3">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-xs text-slate-400 flex items-center gap-1">
                        <Clock className="w-3 h-3" />
                        {ann.created_at ? format(parseISO(ann.created_at), 'dd/MM/yyyy HH:mm') : 'N/A'}
                      </span>
                      <span className="text-xs font-bold text-indigo-600">
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden animate-in zoom-in duration-200">
            ...
            <div className="p-6 max-h-[500px] overflow-y-auto">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {Array.isArray(viewStatus) && viewStatus.map(v => (
